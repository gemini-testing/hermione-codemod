...
const isBlockStatement = node => node.type === c.BLOCK_STATEMENT;
const isReturnStatement = node => node.type === c.RETURN_STATEMENT;
const isExpressionStatement = node => node.type === c.EXPRESSION_STATEMENT;
const isIfStatement = node => node.type === c.IF_STATEMENT;
...
const getNodePosition = node => ({
    start: _.get(node, 'loc.start.line', 0),
    end: _.get(node, 'loc.end.line', 0)
});
...
const filterNodes = node => {
    ...
    if (isIfStatement(node)) {
        return {
            test: getPropsToFindNode(node.test)
        };
    }
...
const getCalleeObjectNames = node => {
    return node.object
        ? [].concat(getCalleeObjectNames(node.object), node.object.name || [])
        : [];
};
