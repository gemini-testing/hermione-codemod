...
const isBrowserExpr = (node, useBrowserName = c.DEFAULT_BROWSER_NAMES) => {
...
const warn = (msg, node, filePath) => {
    const position = _.isArray(node) ? node.map(getNodePosition) : getNodePosition(node);
...
const getBrowserObj = (browser) => {
    return isThisExpr(browser.node.object) ? browser.node : browser.node.object;
};
