...
const {BROWSER_NAME_OPTION, NOT_AWAIT_OPTION, BREAK_CHAINING_ON_OPTION, BROWSER_CHAINING_MODE} = require('../constants');
...
module.exports = class BrowserChainingTransformer extends BaseTransformer {
    ...
    _findBrowserChainingCalls(browser) {
        return this._j(browser)
            .closestAll([this._j.CallExpression, this._j.MemberExpression])
    ...
    _transformChainingCallsToAwait({chainingCallPaths, browser, browserStatementNode, browserStatementParentNode, changeCollectionNode, isRunnable, browserComments, lastBrowserChaining}) {
        const hasChainedCalls = chainingCallPaths.length > 1;
        this._breakChainingCalls(chainingCallPaths);
        ...
        return chainingCallPaths.flatMap((p, i, arr) => {
            ...
            if (!p._isBroken) {
                currNode.callee.object = u.getBrowserObj(browser, this._options[BROWSER_NAME_OPTION]);
            }

            let awaitNode = hasChainedCalls ? this._genAwaitNode({
            ...
            [].concat(awaitNode).forEach((node, i) => {
                ...
                if (!node.loc) {
                    node.loc = _.pick(currNode.loc, ['start', 'end']);
                }
            ...
            if (p._isBroken) {
                awaitNode = this._warnings.wrapNodeToWarns(
                    awaitNode,
                    `found node in browser chaining calls which matches one of the values: ` +
                    `${this._options[BREAK_CHAINING_ON_OPTION]} of option "${BREAK_CHAINING_ON_OPTION}", fix it manually`
                );
            }
    ...
    _breakChainingCalls(browserChainingCallPath = []) {
        const nodePaths = new Set();

        for (const callPath of browserChainingCallPath) {
            const collection = this._j(callPath)
                .find(this._j.MemberExpression, node => {
                    return u.isIdentifier(node.property) && this._options[BREAK_CHAINING_ON_OPTION].includes(node.property.name);
                })
                .filter(p => {
                    return this._j(p)
                        .find(this._j.MemberExpression, node => {
            ...
            const nodePathsSizeBefore = nodePaths.size;
            collection.paths().forEach(p => nodePaths.add(p));

            if (nodePathsSizeBefore < nodePaths.size) {
                callPath._isBroken = true;
            }
        }

        this._j([...nodePaths.values()]).replaceWith(p => p.node.property);
    }
    ...
    _genAwaitNode({browserStatementNode, browserStatementParentNode, changeCollectionNode, isRunnable, isLastChainingCall, currNode, nextNode}) {
        ...
        if (nextResolveCb && u.hasParams(nextResolveCb) && u.isThenNode(nextNode)) {
            const resolveCb = currNode.arguments[0];

            if (resolveCb) {
                this._j(resolveCb)
                    .find(this._j.IfStatement)
                    .filter((p) => u.isBlockStatement(p.parent.node))
                    .forEach(v => {
                        this._warnings.addWarnToSourceCode({problemNode: v.node, parentNode: resolveCb});
                    });
            }
        ...
        if (u.isThenNode(currNode)) {
            ...
            if (isRunnable || !isLastChainingCall) {
                const filterIfStatement = (p) => u.isIfStatement(_.get(p, 'parent.parent.node'));
                this._replaceReturnNodeToAwait({node: modifiedResolveCb, isLastChainingCall, filterCb: filterIfStatement});
            }
    ...
    _replaceReturnNodeToAwait({node, isLastChainingCall, filterCb = () => true}) {
        this._j(node)
    ...
    _removeAwaitFromPassedNodes(fnNode) {
        ...
        const notAwait = _.isString(optionVal) ? optionVal.split(',') : _.concat(optionVal);

        this._j(fnNode)
            .find(this._j.AwaitExpression, node => {
                const callee = _.get(node, 'argument.callee', {});
                const calleeObjNames = u.getCalleeObjectNames(callee);

                return _.intersection(notAwait, [].concat(callee.name, calleeObjNames)).length > 0;
            })
