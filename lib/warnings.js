...
module.exports = class Warnings {
    ...
    addWarnToSourceCode({problemNode, parentNode, propName}) {
        if (this._transformMode === REMOVE_BROWSER_PROP_MODE) {
            if (u.isObjectPattern(problemNode)) {
                this._handleWarn({
                    problemNode,
                    parentNode,
                    warnMsg: `found usage of "${propName}" identifier with other properties inside object pattern, fix it manually`
                });
            }
        ...
        if (u.isLogicalExpr(problemNode) || u.isConditionalExpr(problemNode) || u.isIfStatement(problemNode)) {
            this._handleWarn({
                problemNode,
                parentNode,
    ...
    wrapNodeToWarns(problemNode, warnMsg) {
        this._addInfo(problemNode, warnMsg);
        return this._wrapInSyntaxErrors(problemNode);
    }
    ...
    _addSyntaxErrors(problemNode, parentNode) {
        ...
        if (u.isIfStatement(problemNode)) {
            return this._j(parentNode)
                .find(this._j.IfStatement, u.filterNodes(problemNode))
